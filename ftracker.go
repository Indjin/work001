...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return (float64(runningCaloriesMeanSpeedMultiplier) * ((float64(action) * float64(lenStep) / float64(mInKm) / duration) * float64(runningCaloriesMeanSpeedShift)) * weight / float64(mInKm) * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	//averSpeed := math.Pow((float64(action) * float64(lenStep)), 2)

	averSpeed := meanSpeed(action, duration) * kmhInMsec

	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(averSpeed, 2)/(height/100.0))*walkingSpeedHeightMultiplier*weight) * duration * minInH)

	//return ((float64(walkingCaloriesWeightMultiplier)*weight + (math.Pow((meanSpeed(action, duration)*kmhInMsec), 2)/height)*float64(walkingSpeedHeightMultiplier)*weight) * duration * float64(minInH))
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	return (((float64(lengthPool) * float64(countPool) / 1000.0) / duration) + 1.1) * 2 * weight * duration
}
